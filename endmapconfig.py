...
path_ids method, in which each column is represented as a nonnegative
...
class EndMapConfig(object):
    def __init__(self, initial_config, _copy_config=None, _copy_ends=None):
        if _copy_config is None:
            ...
            self.ends, self.config = EndMapConfig.build_config(initial_config)
        else:
    ...
    def copy(self):
        ...
        c = EndMapConfig(None, _copy_config=new_config, _copy_ends=new_ends)
    ...
    def build_config(config_seq):
        ...
        pairs, loners = EndMapConfig.path_pairs(config_seq)
